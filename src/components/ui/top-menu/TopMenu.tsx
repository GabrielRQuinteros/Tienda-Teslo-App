...
export const TopMenu = () => {
    const { openSideMenu, closeSideMenu, isSideMenuOpen } = useUIStore(
        (state) => state
    );
    ...
    const productsInCartQuantity = useCartStore((state) => state.getTotalItems());
  ...
  return (
    <nav className="flex px-5 justify-between items-center w-full">
      ...
      <div>
        <Link href="/">
          <span className={`${montserAlt.className} antialiased font-bold`}>
            Teslo
          </span>
          <span> | Shop</span>
        </Link>
      ...
      <div className="flex items-center gap-4">
        ...
        <Link href="/cart">
          <div className="relative">
            {isClient && productsInCartQuantity > 0 && (
              <span className="absolute -right-2 -top-2 text-xs rounded-full px-1 font-bold bg-blue-700 text-white">
